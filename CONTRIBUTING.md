...
The Linux coverage job also blocks Python network sockets with `pytest-socket`.
To use that check locally on Linux or macOS, add
`--disable-socket --allow-unix-socket` to pytest. Unix sockets remain available
for the event loop. This does not restrict child processes; media tests launch
only local Python helpers, never FFmpeg against a camera.

See [Architecture](docs/architecture.md) for the command flow and the boundary
between feature logic and transport. Prefer explicit code and existing helpers;
new abstractions or dependencies should remove a concrete maintenance burden.
Requirements may change when their benefit no longer justifies their cost,
but preserve behaviour with an appropriate regression check.
...
Keep editor and development-environment settings, AI assistant instructions
and state, and research files local and ignored. The release checks also reject
these files if they are accidentally tracked or included in a distribution.
