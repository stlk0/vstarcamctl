...
- [CLI guide](cli.md): command groups, common reads, confirmed controls, and
  media tools.
- [Initial Wi-Fi provisioning](initial-wifi-provisioning.md): create the
  experimental local setup QR for a reset camera.
...
- [Safety](safety.md): authorization, secret handling, one-shot writes, and
  physical recovery.
- [Architecture](architecture.md): code boundaries, dependencies, and the
  behaviours contributors should preserve.
- [Releasing](releasing.md): maintainer-only tag, artifact, and Trusted
