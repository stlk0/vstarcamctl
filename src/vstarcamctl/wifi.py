...
from ._normalize import as_int, has_control_chars
...
_SCAN_FIELDS = {
    ...
    "bssid": ("ap_mac", "ap_bssid", "wifi_bssid", "bssid"),
    ...
    # Preserve the firmware field without assigning an undocumented meaning to it.
    "ap_dbm1": ("ap_dbm1",),
}
...
def parse_wifi_scan_response(payload: dict[str, Any]) -> list[dict[str, Any]]:
    """Validate the indexed scan grammar before exposing normalized rows."""

    error = WifiConfigurationError
    folded = _casefolded(payload)
    if as_int(folded.get("result"), "result", error) != 0:
        raise error("Wi-Fi scan failed")
    count = as_int(folded.get("ap_number"), "ap_number", error)
    if count < 0:
        raise error("ap_number must be non-negative")
    ...
    for output, candidates in _SCAN_FIELDS.items():
        ...
        if not isinstance(value, list):
            raise error(f"Wi-Fi scan field {candidates[0]} must be an array")
        if len(value) != count:
            raise error(f"Wi-Fi scan field {candidates[0]} does not match ap_number")
        columns[output] = value

    missing = {"ssid", "channel", "auth_type"} - columns.keys()
    if missing:
        raise error("Wi-Fi scan is missing required arrays: " + ", ".join(sorted(missing)))
    return normalize_wifi_scan(payload)
...
def complete_wifi_metadata(
    ...
    if not matches:
        raise WifiConfigurationError(
            "SSID and supplied metadata do not exactly match the camera scan"
        )
